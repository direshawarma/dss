...
#[async_trait]
trait Ffff {
    // I don't understand what I'm doing so I don't have a good name for this trait
    async fn write_episode(&self);
}
...
pub async fn db_add(series_title: &String, title: String, url: String) {
    ...
    for cap in SEASONREGEX.captures_iter(&entry.title.to_string()) {
        ...
        entry.title = SEASONREGEX
            .replace(&entry.title, "$before $after")
            .to_string();
    }
    for cap in EPISODEREGEX.captures_iter(&entry.title.to_string()) {
        entry.episode = cap[1].parse::<i32>().unwrap();
        entry.title = EPISODEREGEX
            .replace(&entry.title, "$before $after")
            .to_string();
    }

    entry.series = SPACEKILLER
        .replace(&entry.title, "$before $after")
        .to_string();
    entry.title = BEGINFIXER
        .replace(&entry.title, |caps: &Captures| format!("{}", &caps[1]))
        .to_string();
